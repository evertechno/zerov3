...
# New session state for advanced weighting raw metrics before final weights are assigned
if "advanced_weighting_raw_metrics" not in st.session_state:
    st.session_state["advanced_weighting_raw_metrics"] = pd.DataFrame()
...
def render_custom_index_tab(kite_client: KiteConnect | None, supabase_client: Client, api_key: str | None, access_token: str | None):
    ...
    with create_tabs[3]: # Advanced Weighting
        ...
        st.info("Enter stock symbols, select a weighting strategy, and then 'Validate & Auto-Assign Weights'. You can then review and apply them.")
        symbols_input = st.text_area("Enter stock symbols (comma-separated)", "RELIANCE,TCS,INFY,HDFCBANK,ICICIBANK,HINDUNILVR,ITC,BHARTIARTL,SBIN,LICI", height=100, key="adv_weights_symbols_input")
        strategy = st.selectbox("Select Weighting Strategy", ["Inverse Volatility (30d)", "Momentum (6m)"], key="adv_weights_strategy_select")
        
        # New button for validation and auto-assignment
        if st.button("Validate & Auto-Assign Weights", key="generate_adv_weights_btn"):
            symbols = [s.strip().upper() for s in symbols_input.split(',') if s.strip()]
            if not symbols:
                st.warning("Please enter at least one symbol.")
            else:
                with st.spinner(f"Calculating {strategy} metrics for {len(symbols)} symbols..."):
                    metrics = {}
                        ...
                        if "_error" not in hist_df.columns and not hist_df.empty and not hist_df['close'].empty:
                            if strategy == "Momentum (6m)":
                                if len(hist_df) >= 2: # Need at least two data points for momentum
                                    ...
                                    metrics[symbol] = momentum # Store raw momentum, positive or negative
                                else:
                                    st.warning(f"Skipping {symbol}: Insufficient data for Momentum calculation.")
                                ...
                                if not returns.empty:
                                    ...
                                    st.warning(f"Skipping {symbol}: Insufficient data for Volatility calculation.")
                        else:
                    ...
                    if not metrics:
                        ...
                        results_df = pd.DataFrame(list(metrics.items()), columns=['symbol', 'Metric Value'])
                        results_df['Name'] = results_df['symbol'] # Default name
                        st.session_state["advanced_weighting_raw_metrics"] = results_df
                        st.success(f"{strategy} metrics calculated. Review and apply below.")

        adv_raw_metrics_df = st.session_state.get("advanced_weighting_raw_metrics")
        if adv_raw_metrics_df is not None and not adv_raw_metrics_df.empty:
            st.markdown("##### Review and Adjust Calculated Factor Values")
            st.info("You can edit the 'Metric Value' for each symbol before applying. Negative values will be treated as zero when calculating final weights if 'Remove negative factors' is checked.")

            edited_metrics_df = st.data_editor(
                adv_raw_metrics_df,
                key="edited_adv_metrics_editor",
                num_rows="fixed",
                column_config={
                    "Metric Value": st.column_config.NumberColumn(
                        "Metric Value",
                        help="Calculated factor for weighting (e.g., Inverse Volatility, Momentum). Adjust as needed.",
                        format="%.6f",
                    ),
            ...
            remove_negative_factors = st.checkbox("Remove negative factors (treat as zero)", value=True, key="remove_neg_adv_factors")
            
            if st.button("Apply Calculated Weights", key="apply_adv_weights_btn"):
                df_to_weight = edited_metrics_df.copy()
                df_to_weight["Metric Value"] = pd.to_numeric(df_to_weight["Metric Value"], errors='coerce').fillna(0)

                if remove_negative_factors:
                    df_to_weight['Metric Value'] = df_to_weight['Metric Value'].apply(lambda x: max(0, x))

                total_metric_value = df_to_weight['Metric Value'].sum()
                
                if total_metric_value > 0:
                    df_to_weight['Weights'] = df_to_weight['Metric Value'] / total_metric_value
                    processed_df_final_weights = df_to_weight[['symbol', 'Name', 'Weights']]
                    
                    st.session_state["current_calculated_index_data"] = processed_df_final_weights
                    ...
                    st.success("Advanced weights applied and ready for historical calculation.")
                    st.session_state["advanced_weighting_raw_metrics"] = pd.DataFrame() # Clear raw metrics
                else:
                    st.error("Total metric value after adjustments is zero. Cannot generate weights. Please adjust factors.")
