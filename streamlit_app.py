...
import warnings
# Suppress the FutureWarning from LightGBM
...
# --- CRITICAL DISCLAIMER ---
# Acknowledging the request for near 100% precision, but maintaining financial reality.
st.warning("""
DISCLAIMER: This platform employs state-of-the-art algorithms (LightGBM) and deep feature engineering 
to achieve maximum predictive power. **Note that guaranteed 100% precision is impossible in real financial markets.** 
The 'Precision' metric below refers to Directional Accuracy (predicting up/down movement).
""")
st.markdown("---")
...
# Use cache_data decorator for secrets function to prevent re-reading
...
def load_secrets():
    ...
    # Supabase check kept as per original user template
...
def get_historical_data_cached(api_key: str, access_token: str, symbol: str, from_date: datetime.date, to_date: datetime.date, interval: str, exchange: str = DEFAULT_EXCHANGE) -> pd.DataFrame:
    ...
    try:
        # Note: Kite historical data only works up to 60 days for minute intervals
...
with st.sidebar:
    ...
    if st.session_state.get("ml_model"):
        ...
        st.info(f"Horizon: {st.session_state['prediction_horizon']} periods")
    else:
...
def render_price_predictor_tab(kite_client: KiteConnect | None, api_key: str | None, access_token: str | None):
    ...
    if not ml_data.empty:
        ...
        with col_prep:
            current_prediction_horizon = st.number_input("Prediction Horizon (Periods/Days Ahead)", min_value=1, max_value=20, value=st.session_state.get("prediction_horizon", 5), step=1, key="pred_horizon")
        ...
        # Define the target: The close price shifted back by the horizon
        ...
        # Features exclude price columns, target, and volume 
        ...
        with col_ml_controls:
            ...
            # Default selection of robust features
            ...
            if X.empty or y.empty or len(X) < 100:
                st.error(f"Insufficient clean data for robust training (need at least 100 samples, got {len(X)}).")
                return
            ...
            # Split data (shuffle=False for time series integrity)
            ...
            if st.button(f"Train {model_type_selected} Model", key="train_ml_model_btn"):
                ...
                model = {
                    ...
                    "LightGBM Regressor (High Performance)": lgb.LGBMRegressor(n_estimators=500, learning_rate=0.03, num_leaves=40, random_state=42, n_jobs=-1)
                }.get(model_type_selected)

                if model:
                    with st.spinner(f"Training {model_type_selected} model for {current_prediction_horizon}-period prediction..."):
                        model.fit(X_train, y_train)
                        y_pred = model.predict(X_test)
                    
                    # Store everything needed for evaluation and forecasting
                    ...
                    st.success(f"**{model_type_selected}** Model Trained for {current_prediction_horizon}-period horizon!")
        ...
        with col_ml_output:
            if st.session_state.get("ml_model") and st.session_state.get("y_test") is not None:
                ...
                # --- FIX FOR INDEX ERROR: Use .loc on ml_data ---
                # Get the 'close' price that was current when the features (X_test) were measured.
                # This price is correctly located by using the index of y_test on the ml_data DataFrame.
                base_close_prices = ml_data.loc[y_test_s.index]['close'].values
                
                # Calculate directional movement relative to the base price
                actual_direction = np.sign(y_test_s.values - base_close_prices)
                predicted_direction = np.sign(y_pred_s - base_close_prices)
                
                # Directional Accuracy (Precision)
        ...
        if st.session_state.get("ml_model") and not ml_data.empty:
            ...
            # Use the absolute last row of the ML data (which is the current, latest known state)
            latest_row = ml_data[features_list].iloc[[-1]] 
            
            # Scale the features
            if scaler is None:
                st.error("Scaler object not found. Please retrain the model.")
                return
            ...
            if st.button(f"Generate Forecast for Next {horizon} Periods ({model_type_selected})", key="generate_forecast_btn"):
                ...
                # Calculate approximate date of prediction fulfillment
                try:
                    # Attempt to use the frequency if available (for minute/hour data)
                ...
                if predicted_change >= 1.5:
                    st.success(f"**STRONG BUY:** High confidence in significant upward momentum ({predicted_change:.2f}%) over the next {horizon} periods.")
                elif predicted_change > 0.1:
                    st.info(f"**BUY/HOLD:** Potential upward movement expected ({predicted_change:.2f}%).")
                elif predicted_change <= -1.5:
                    st.error(f"**STRONG SELL/SHORT:** High confidence in significant downward momentum ({abs(predicted_change):.2f}%) over the next {horizon} periods.")
                elif predicted_change < -0.1:
                    st.warning(f"**SELL/WAIT:** Potential downward movement expected ({abs(predicted_change):.2f}%).")
                else:
                    st.markdown(f"**NEUTRAL:** Expecting minor price fluctuation (change within ±0.1%).")
        else:
